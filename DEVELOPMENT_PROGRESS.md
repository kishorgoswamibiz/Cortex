...
**Session 5: Smart capture — two-stage pipeline, temporal resolution, robust tagging, reminders, premium polish**

This session added a personal-assistant layer on top of the existing graph: every capture now resolves dates to absolute timestamps, tags entities far more accurately, and can raise exact-alarm reminders surfaced on a calendar. The DB went to **v2** with a real (non-destructive) migration. `assembleDebug` is green and the JVM unit tests pass.

- **DB v2 (`data/`).** `ItemEntity` gained `due_at`, `remind_at`, `temporal_precision`, `temporal_phrase`; `CaptureEntity` gained `zone_id` (captured at enqueue so relative dates resolve against the user's wall-clock even if the worker runs later/after travel/DST); `NodeEntity` gained `phonetic_key`; new `ReminderEntity` + `NodeAliasEntity`; FK indices added. `AppDatabase` bumped to version 2 with `MIGRATION_1_2` (additive ALTERs + new tables), removed `fallbackToDestructiveMigration`, and turned on `exportSchema` (KSP `room.schemaLocation` set in `app/build.gradle.kts`). New DAO queries for reminders, dated items, aliases (with an `ON CONFLICT … DO UPDATE` upsert), phonetic/name-key projection, and stale-embedding lookup.
- **Two-stage pipeline.** Stage A `NormalizeService` (`pipeline/`) calls the flash model to clean spelling/grammar and resolve every temporal expression to ISO using injected `NOW`/`ZONE` (variable data kept in the **user** message so the system prompt stays cache-stable). A pure-`java.time` `TemporalResolver` independently parses the common grammar (tomorrow, next Tuesday, in 2 weeks, after a month, on the 5th, July 3 at 5pm, end of month, …) and `TemporalReconciler` cross-checks the two: on disagreement the deterministic value wins, is demoted to day-precision, and flagged; offline it runs deterministic-only. Covered by `TemporalResolverTest` + `TemporalReconcilerTest`.
- **Robust tagging (Stage B).** Bundled the real int8 `all-MiniLM-L6-v2` ONNX + `vocab.txt` under `assets/embeddings/` (verified inputs `input_ids/attention_mask/token_type_ids`, output `last_hidden_state[1,seq,384]` — matches `EmbeddingService`'s mean-pool), so `modelVersion` flips off `fallback-hash`. `ReEmbedWorker` upgrades old fallback vectors; retrieval/resolution filter to the current model version so the two never mix. `CandidateRetrieval` rewritten to **hybrid multi-recall + RRF** (LIKE ∪ trigram-fuzzy ∪ phonetic ∪ alias ∪ vector-KNN; `FuzzyMatch` + compact `Phonetic`; `NameNormalizer` shared); dropped the harmful recent-node padding. `EntityResolver` now blends name+phonetic+vector with type as a prior, and **only auto-merges with real embeddings and no name ambiguity** (else routes to confirmation). `ExtractionApplier` attaches the temporal anchor, creates and schedules reminders, stamps phonetic keys, learns aliases (post-commit, with garbage/collision gates), and fixes the `relatedNodeId` first-link bug. Prompt/schema extended with `TODAY` + signal-annotated candidates; model id centralized in `DeepSeekClient.MODEL` (still `deepseek-chat` until `deepseek-v4-flash` is verified live).
- **Reminders subsystem (`reminders/`).** `ReminderScheduler` (exact `setExactAndAllowWhileIdle`, inexact fallback if not permitted), `ReminderReceiver` (notification with Done/Snooze, marks status), `BootReceiver` (re-schedules after reboot), `Notifications` (channel + builder). Manifest gained `POST_NOTIFICATIONS`/`SCHEDULE_EXACT_ALARM`/`USE_EXACT_ALARM`/`RECEIVE_BOOT_COMPLETED` + receivers; `MainActivity` requests notifications at runtime, ensures the channel, and enqueues `ReEmbedWorker`.
- **Calendar UI (`ui/screens/ReminderCalendarScreen.kt`).** Glass month grid with per-day domain dots, default-selects today, taps to a chronological day list (reminders + dated items) with complete/snooze/open-node. Added the **Reminders** destination to `AppShell` nav.
- **Premium polish.** `glassSurface` upgraded with a soft drop shadow + top sheen gradient + highlight hairline (reads as floating frosted glass on linen); gentle cross-fade transitions on nav; haptic tap on reminder completion.
- **Known limitations / next:** model file (~22 MB) is committed under assets — flip `DeepSeekClient.MODEL` to `deepseek-v4-flash` once verified; multi-date-per-note persists only the primary anchor (others logged/deferred); recurrence is captured as a flag but not yet scheduled; the medium-confidence confirmation inbox is counted but still needs a UI; on-device end-to-end (reminder firing, MiniLM active in Settings) still to be verified on the device.
